...
if (rc_CTIME > 0)
    if (rc_CTIME == 1)
       ...
       prev_e = [ein3(rc_CTIME) 200 200]';
        
    elseif  (rc_CTIME == 2)
        prev_e = [ein3(rc_CTIME - 1) ein3(rc_CTIME) 200]';
    elseif  (rc_CTIME == 3)
        prev_e = [ein3(rc_CTIME - 2) ein3(rc_CTIME - 1) ein3(rc_CTIME)]';
        %prev_e = ein3(rc_CTIME-pred_span+1:rc_CTIME);
    else
        prev_e = ein3(rc_CTIME-3:rc_CTIME-1);
    end
end
